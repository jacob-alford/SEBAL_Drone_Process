...
function setup() {
  ...
  tf.tidy(() => {
    ...
    // --- Basic Calculations ---
    let ndvi = nirTensor.sub(redsTensor).div(nirTensor.add(redsTensor)).mul(255); // Normalized-Difference Vegetation Index
    let savi = nirTensor.sub(redsTensor).mul(1.1).div(nirTensor.add(redsTensor).add(.1)).mul(255); // Not Sure
    let lai = savi.div(255).mul(-1).add(.69).div(.59).log().div(.91); // Not sure
    let emissivity = lai.mul(.01).add(.95); // Surface Emissivity
    let [rgbRTensor,rgbGTensor,rgbBTensor] = tf.split(rgbTensor,3,2); // Seperate tensors with red-blue-green values
    ...
    let rawSoil1 = soilSaturation1.toInt().dataSync();
    let rawSoil2 = soilSaturation2.toInt().dataSync();
